...
def current_meal(day):
    ...
    if weekday == True:
        cursor.execute("SELECT * FROM DiningHallSchedules WHERE diningHallName = 'sage' AND (weekDay = 'True' OR weekDay IS NULL)")
    else:
        cursor.execute("SELECT * FROM DiningHallSchedules WHERE diningHallName = 'sage' AND (weekDay = 'False' OR weekDay IS NULL)")
...
def parse_html(request):
    ...
    return render(request, 'russellsage.html', {'current_meals': current_meals})
