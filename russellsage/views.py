...
def russellsage(request):
   ...
   lunch_items = FoodItem.objects.using('PostgresDB').filter(dining_hall="russellsage", meal='LUNCH').order_by('station', 'name')
   dinner_items = FoodItem.objects.using('PostgresDB').filter(dining_hall="russellsage", meal='DINNER').order_by('station', 'name')
  
   # Pass the result to the template
   return render(request, 'russellsage.html', {'breakfast_items': breakfast_items, 'lunch_items': lunch_items, 'dinner_items': dinner_items})
