...
def blitman(request):
   #populate(url = "https://rpi.sodexomyway.com/en-us/locations/blitman-dining-hall", dining_hall = "blitman")


   breakfast_items = FoodItem.objects.using('PostgresDB').filter(dining_hall="blitman", meal='BREAKFAST').order_by('station', 'name')
   lunch_items = FoodItem.objects.using('PostgresDB').filter(dining_hall="blitman", meal='LUNCH').order_by('station', 'name')
   dinner_items = FoodItem.objects.using('PostgresDB').filter(dining_hall="blitman", meal='DINNER').order_by('station', 'name')
  
   # Pass the result to the template
   return render(request, 'blitman.html', {'breakfast_items': breakfast_items, 'lunch_items': lunch_items, 'dinner_items': dinner_items})
